<template>
  <div class="relative mt-12 flex h-[80vh] flex-col justify-center gap-20 px-5">
    <form @submit.prevent="createArtistepref">
...
export default {
  ...
  methods: {
    ...
    async createArtistepref() {
      const storage = getStorage();
      const refStorage = ref(storage, "artistepref/" + this.artistepref.image);
      console.log("refStorage", refStorage);
      await uploadString(refStorage, this.imageData, "data_url").then((snapshot) => {
        console.log("Uploaded a base64 string");
        const db = getFirestore();
        const docRef = addDoc(collection(db, "artistepref"), this.artistepref);
      });
